...
public:
    ...
    explicit Vec(const Vec<T, N2> &ohter) {
        int i = 0;
    ...
    template <typename U, uint32_t N2>
    Vec(const Vec<U, N2> &other, T value) {
        int i = 0;
        for (; i < std::min(N, N2); i++) {
...
template <typename T>
class Vec<T, 2> final : public CommonVecOperations<T, 2> {
public:
    union {
        struct {
            T x, y;
...
template <typename T>
class Vec<T, 3> final : public CommonVecOperations<T, 3> {
public:
    union {
        struct {
            T x, y, z;
...
template <typename T>
class Vec<T, 4> final : public CommonVecOperations<T, 4> {
public:
    union {
        struct {
            T x, y, z, w;
...
inline float UChar2Float(const unsigned char c) {
