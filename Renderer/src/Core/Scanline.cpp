...
Trapezoid::FromTriangle(const Vertex (&vertices)[3]) {
    // clang-format on
    ...
    if (t_Vertices[0].position.y == t_Vertices[1].position.y) {
        ...
        auto trap = Trapezoid{
            t_Vertices[0].position.y, t_Vertices[2].position.y,
    ...
    if (x > t_Vertices[1].position.x) {
        auto trap_1 = Trapezoid{
            t_Vertices[0].position.y, t_Vertices[1].position.y,
            Edge{t_Vertices[0], t_Vertices[1]},
            Edge{t_Vertices[0], t_Vertices[2]}
        };

        auto trap_2 = Trapezoid{
            t_Vertices[1].position.y, t_Vertices[2].position.y,
            Edge{t_Vertices[1], t_Vertices[2]},
            Edge{t_Vertices[0], t_Vertices[2]}
        };
...
Scanline::Scanline(const Vertex &vertex, const Vertex &step, const float init_Y,
                   const float width)
    : vertex(vertex), step(step), y(init_Y), width(width) {}

Scanline Scanline::FromTrapzoid(const Trapezoid &trap, const float init_Y) {
