...
#include "Core/CohenSutherland.hpp"

namespace Rasterization {
Bresenham::Bresenham(Vec2 &point_1, Vec2 &point_2, const Vec2 &rect_min,
                     const Vec2 &rect_max) {
...
void Bresenham::IteratorCallback(
    std::function<void(int32_t x, int32_t y)> callback) {
    while (x != final_X) {
        if (steep > 0) {
            callback(y, x);
        } else {
